 ...
 *
 * 三个 Tab（融合 shell.html 的设计精华）：
 *   "控制台" — 角色卡片列表 / 调度按钮 / 最近日志
 *   "数据"   — 聊天消息 / 世界书命中 / 破限预览
 *   "配置"   — 模型路由 / 破限文本 / 世界书绑定 / 导出导入
 *
 * 交互改进（基于 shell.html 审查反馈）：
 *   - 角色状态：文字 + 颜色双重表达
 *   - 危险操作：二次确认
 *   - 长文本：点击展开/收起
 *   - 面板偏好：localStorage 持久化
 *
 * 由 bootstrap.ts 在 API 装配完毕后调用 injectFloatingPanel()。
 */
...
`.trim();
  ...
  root.innerHTML = `
<button id="td-fab" title="酒馆导演台"><img src="${FAB_IMG}" alt="导演" width="48" height="48" /></button>
<div id="td-panel">
  <div id="td-header">
  ...
  interface PanelState {
    ...
    collapsed: boolean;
    currentTab: 'console' | 'data' | 'settings';
    worldBooks: Array<{ id: string; title: string; keys: string[]; content: string; enabled: boolean; hit: boolean; hitReason: string; }>;
    jailbreak: { text: string; source: string; enabled: boolean; name: string };
    messages: Array<{ id: string; role: string; speaker: string; content: string; turnIndex: number; isDirectorDecision: boolean }>;
    characters: Array<{ id: string; name: string; status: string; isNarrator: boolean; isSelected: boolean; model: string; avatar: string }>;
    logs: Array<{ timestamp: number; selectedRoles: string[]; orderedRoles: string[]; skippedRoles: string[]; reason: string; mode: string }>;
  }
  ...
  function loadFabPos() {
    try {
      const saved = localStorage.getItem('td-fab-pos');
      if (saved) {
  ...
  // FAB drag
  let fabDragging = false, fabStartX = 0, fabStartY = 0, fabOrigLeft = 0, fabOrigTop = 0;
  ...
  function fabDragMove(e) {
    ...
    const p = 'touches' in e ? e.touches[0] : e;
    const dx = p.clientX - fabStartX;
    const dy = p.clientY - fabStartY;
  ...
  $tabs.forEach(t => {
    if ((t as HTMLElement).dataset.tab === S.currentTab) {
      $tabs.forEach(tt => tt.classList.remove('active'));
      t.classList.add('active');
    }
  });
  ...
  function syncData() {
    ...
    try {
      ...
      S.messages = (snap.messages || []).slice(-50).map((m: any, i: number) => ({
        id: m.id || `m_${i}`,
        role: m.role || 'system',
        speaker: m.speaker || '',
        content: m.content || '',
